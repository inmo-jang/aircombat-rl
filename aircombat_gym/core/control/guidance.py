"""Guidance: (heading, speed, altitude) targets -> stick and throttle.
...
class Guidance:
    ...
    def __init__(self, h0_ft: float = H0_FT, gains: GuidanceGains | None = None,
    ...
    def update(self, s, psi_cmd: float, v_cmd_kt: float, dt: float,
               alt_cmd_ft: float | None = None) -> GuidanceOutput:
        """s is an AircraftState.  psi_cmd rad, v_cmd_kt knots, alt_cmd_ft feet.

        `alt_cmd_ft` defaults to the altitude this guidance was built with, so
        ...
        return GuidanceOutput(aileron=aileron, elevator=elevator, rudder=0.0,
